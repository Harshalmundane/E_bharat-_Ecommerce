...
import Modal from '../model/Model'; // Ensure Modal is imported correctly
    ...
    const options = {
      ...
      handler: async function (response) {
        ...
        try {
          ...
          console.log(error);
          toast.error("Error storing order in database.");
        }
      },
      theme: {
        color: "#3399cc",
      ...
      <div className="h-screen bg-gray-100 pt-5" style={{ backgroundColor: mode === 'dark' ? '#282c34' : '', color: mode === 'dark' ? 'white' : '' }}>
              ...
              cartItems.map((item, index) => (
                <div key={index} className="justify-between mb-6 rounded-lg border drop-shadow-xl bg-white p-6 sm:flex sm:justify-start" style={{ backgroundColor: mode === 'dark' ? 'rgb(32 33 34)' : '', color: mode === 'dark' ? 'white' : '' }}>
                  ...
                  <div className="sm:ml-4 sm:flex sm:w-full sm:justify-between">
                    ...
                    <div onClick={(e) => deleteCart(item, e)} className="mt-4 flex justify-between sm:space-y-6 sm:mt-0 sm:block sm:space-x-6">
        ...
        {modalOpen && (
          <Modal
            ...
            buyNow={buyNow}
            onClose={() => setModalOpen(false)} // Close modal on close
          />
        )}
      </div>
    </Layout>
